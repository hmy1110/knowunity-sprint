...
3. **No Pressed feedback, no motion.** Craft, G4. Held-down pairs 0.000%, `C03-a` vs `C03-b` 0.000%, no running animations in any loop state. Fix: drive `state="Pressed"` from pointer-down, animate the recording waveform under `prefers-reduced-motion`. Motion durations need a token Mia has to choose.
