#!/bin/bash
# Command logs for evidence-03. Writes eval/evidence-03/logs/*. Build is run separately (build.sh), last.
...
# grep-hex-raw.txt: hex literals on lines that are not comment-only. This reproduces run 2's grep-hex-raw.txt result on run 2's tree (2 lines). grep-hex-all-lines.txt: every line incl. comments.
