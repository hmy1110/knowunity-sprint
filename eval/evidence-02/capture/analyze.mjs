// Post-capture analysis for evidence-02: pixel diffs, contrast, hit areas, structure.
// Reads screens/ + measure/ written by capture.mjs. Render and measure only. No scoring.
// Usage: node eval/evidence-02/capture/analyze.mjs
import fs from 'node:fs'
import { OUT, RUN1, PNG } from './lib.mjs'

const rd = (p) => JSON.parse(fs.readFileSync(p, 'utf8'))
let manifest = rd(`${OUT}/manifest.json`)
// tag crops are files of B01, not states: keep run 1's manifest shape (state entries only)
...
const run1Pairs = rd(`${RUN1}/measure/pixel-diffs.json`).diffs.map((d) => ({ a: d.a, b: d.b, tag: d.tag }))
const extraPairs = [
  { a: 'X01-t1-typeInput', b: 'X02-t1-typeInput-filled', tag: 'run-2 addition: term 1 typeInput vs filled' },
  { a: 'X02-t1-typeInput-filled', b: 'F07-t1-typeProcessing', tag: 'run-2 addition: term 1 Submit Default vs Disabled' },
  { a: 'C01-t1-idle', b: 'X01-t1-typeInput', tag: 'run-2 addition: term 1 idle vs typeInput' },
  { a: 'F03-t2-idle-before-type', b: 'F05-t2-typeInput', tag: 'run-2 addition: term 2 idle vs after "Type instead" tap' },
  { a: 'F03-t2-idle-before-type', b: 'F06b-t2-typeProcessing-7s', tag: 'run-2 addition: term 2 idle vs 7s after "Type instead" tap' },
]
const pathOf = (n) => `${OUT}/screens/${n}.png`
const diffs = []
fs.mkdirSync(`${OUT}/diffs`, { recursive: true })
for (const p of [...run1Pairs, ...extraPairs]) {
...
// ------------------------------------------------------------------ Submit default vs disabled (term 1 stand-in)
{
  ...
  fs.writeFileSync(`${OUT}/measure/submit-default-vs-disabled.json`, JSON.stringify({
    note: `G4 pair 'Primary button Default vs Disabled'. Crop of the Submit button box (x=${box.x},y=${box.y},${box.width}x${box.height} css px) from two screenshots; per-channel tolerance 8. Run 2 uses term 1 (run 1 used term 2, where the typed path no longer exists).`,
...
for (const id of ids) {
  ...
  contrast[id] = m.texts.map((t) => {
    ...
    let s = null
    const inView = rect.y >= 0 && rect.y + rect.h <= 844 && rect.x >= 0 && rect.x + rect.w <= 390
