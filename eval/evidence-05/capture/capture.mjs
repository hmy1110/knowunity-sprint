// Evidence 05 capture. Drives the real UI at http://localhost:3000 through the free voice/type flow.
// Usage: node eval/evidence-05/capture/capture.mjs   Render and measure only. No scoring.
...
await step('F-denied', async () => {
  await R.goto('/session'); await R.btn('Tap to speak').click(); await sleep(1200); await snap('F04-mic-denied-after-tap')
})
...
// ---------------------------------------------------------------------------
// Added for evidence-05, to close the gaps scorecard-04 listed under
