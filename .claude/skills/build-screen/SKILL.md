...
2. **Check whether this screen has a frame in the Figma file** (`Yummy__Knowie Design System`, fileKey `Km4r5Waxhm1bysmCcFNYiB`) using the Figma MCP tools (`get_design_context`, `get_screenshot`, `get_metadata`). Some screens have a real instance in Figma (e.g. Primer-intro, Learning-result-Hinted2-succeed); some are marked in SPEC.md/design-system.md as non-Figma or inferred-from-structure-only (e.g. `ScoreBreakdown`, `TermResultList`, the badge/XP counter). This changes what "done" means:
   - **Has a frame:** build the screen exactly like the Figma file — layout, spacing, copy, states, every element the frame actually shows. Pull the real values via the Figma tools rather than eyeballing a screenshot. SPEC.md's own "Components" list for a screen is not an exhaustive scope boundary — it's prose written from an earlier pass over the file, and it can under-describe or go stale (missing elements, wrong variant, wrong copy) relative to the live frame. When the live frame shows something SPEC.md doesn't mention, or shows something different from what SPEC.md says, build to match the live frame and flag the gap in `component-gaps.md` — don't skip it because SPEC.md didn't name it, and don't silently keep SPEC.md's version because it was written down first. Use SPEC.md for behavior, state transitions, and destinations (things a static frame can't show), and the live frame as the visual source of truth for everything actually on screen.
