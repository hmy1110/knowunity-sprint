...
Source: live audit of the Figma file "Yummy__Knowie Design System" (fileKey `Km4r5Waxhm1bysmCcFNYiB`), the 16 component sets on the 🎨  Mascot & components page (10 original, plus 6 built for the voice active-recall sprint between 2026-09-11 and 2026-09-12: audioScrubber, inlineAlert, micButton, speechBubble, statusIndicator, tag, covered in Section 5), real usage across ✨  Example Screens, and the Design Brief and Platform Constraints docs.
...
**appBar** — Top nav row: back or menu icon button, a stretchable content slot, optional right button(s). Confirmed in production as of 2026-09-11: the recall loop's Primer-micDenied screen and every Learning-* screen now use appBar's leftIconButtonOnly variant for their top row, replacing the hand-built row they used before. The Slot holds two children side by side: a progressIndicator set to fill the width, and the hand-built XP/lightning counter set to hug (see the new gap flagged after chips below). That confirms the stretchable slot can hold more than a single title. Primer-intro and Summary reach appBar through their own scaffold's Slot - Top navigation rather than a bare instance placed directly on the screen. See the new open item in Section 2 about that scaffold.

**button** — The app's main CTA, confirmed heavily in real use (quiz "Check"/"Continue", "Try again", explain-out-loud's "Continue"). Use Primary/L for the loop's main forward action, Tertiary for lighter text actions like "Why?" or "Skip". Only Default and Pressed states have real examples anywhere. Loading is untested and is exactly the state the processing-wait screen needs, so build and screenshot it before you rely on it.
...
**chips** — Real screens use chips constantly: topic tags, tool pickers (Coach Me/Solve It/Lock In), status tags (PRO). This is the right shape for the summary screen's per-term status labels (unaided, hinted, revealed, skipped). But its color property only offers Primary/pro today, and the summary screen needs an Info/Success/Error/Neutral vocabulary a green "unaided" tag has to read differently from a red "skipped" one. Flag the color property gap before building the summary screen on top of this component; don't fake the semantics with Primary alone.
...
**mascotSlot** — The closest thing to a confirmed hero component for this feature. It already appears in the real explain-out-loud beta doing exactly this job: idle prompt, thinking/processing state, correct checkmark, "Try again" miss state, all through Knowie's expression. 2XL and 3XL dominate real usage; XL is never used. Default the recall loop's Knowie to 2XL or 3XL and cross-check against the beta's own scale before locking your layout.

**progressIndicator** — Directly confirmed in the beta: the "1/3", "2/3" top bar is this component's `showText=true` unit-count mode, and that's the pattern to reuse for the loop's per-term progress (matching the brief's 3-5 term sessions). Only Primary color and progress values 0 and 25 have real examples anywhere (7 total instances of this component exist), so Coral and progress=100 are genuinely untested; confirm any use of them is a deliberate choice, not a guess.
...
## 5. Recall loop components (voice active-recall sprint, built 2026-09-11 to 2026-09-12)

Six new component sets, built for the voice-based active-recall feature and reviewed against real hand-built precedent wherever one existed. Each one's full description below is reproduced verbatim from its Figma component-set description (What it is / When to use it / One thing to do); read those descriptions in Figma directly if this file and the file ever disagree, since Figma is the source of truth.
...
When to use it: This backs "listen once more before deciding," the ability to replay a recorded answer before submitting it, and general audio playback anywhere the loop needs it (prompt readback, recorded-answer review). It now has a real instance in Learning-review, placed above the status row and Redo/Send buttons.

One thing to do: Fixed a real production bug on 3 live screens (Learning-idle, Learning-processing, Learning-resultNeedsPractice) where an unbound white 24x24 square sat behind the play icon, a leftover from hand-building rather than something intentional. Those 3 screens were patched in place (transparent fill) but still use the old hand-built "Input" bubble pattern rather than a real audioScrubber instance; swap them for real instances so future fixes propagate instead of needing to be found and patched again.
...
When to use it: This is the tap-to-speak trigger for the voice active-recall loop, the moment a student starts and stops answering a term out loud. It now has a real instance in Learning-recording, in its Recording state. Idle and Pressed still have no real-screen instance; the explain-out-loud beta screens hand-build a large circular mic affordance, but at a size and interaction model not confirmed to match this component.

One thing to do: Processing still has no real-screen precedent to test against, since nothing existing shows what "transcribing" looks like mid-interaction. Confirm the intended interaction (tap-to-toggle vs. hold-to-record) before wiring transitions, since that decides what Pressed leads into, and check Processing's timing against actual speech-to-text latency once it's built.

**speechBubble** — states (`state`): Prompt, Success, Warning, Error. Other properties: `showSubtitle` (boolean, default true).

What it is: 4-state feedback bubble (Prompt/Success/Warning/Error) with a tail, an optional showSubtitle toggle, and a Header (icon + colored title + subtitle) above a message. Success/Warning/Error icon and title are bound to feedback/success/bold, text/warning, and feedback/error/bold respectively (corrected from an initial accent/green/bold on Success).

When to use it: This is Knowie's spoken/written response bubble in the recall loop, the thing a student reads after answering. The tail+bubble shape is already hand-built 11 times across live screens; this formalizes it as one component.

One thing to do: The Header's icon+title+subtitle structure duplicates what's now built separately as inlineAlert, and their optional-text layers are named differently (Subtitle here, Descriptor there). Decide whether speechBubble's Success/Warning/Error headers should embed an inlineAlert instance instead of their own copy of the same pattern, since keeping both means every future header change has to be made twice.
...
When to use it: This is now the single source for a pattern that used to be hand-built independently in 3 places. It's live in 2 real screens: embedded inside micButton's own Recording state (paired with "Tap to stop"), and placed directly in Learning-review showing "Ready to send" above the Redo/Send buttons. Both usages are correctly bound to feedback/error/bold and feedback/success/bold. Two older duplicate copies still sit in Archive, untouched since those are superseded screen versions, not live ones.
...
Also worth knowing: Learning-recording briefly had this rendering twice (once standalone above the mic button, once inside micButton's own Recording state) after the hand-built rows were first swapped for real instances. The standalone instance has since been removed, keeping only the one embedded in micButton.
...
When to use it: This is the per-term result label on the summary screen after a recall session. It's a new standalone component, not an extension of the real chips component: chips is a 16-variant primitive (size XXS-M, color Primary/pro, mandatory dual icon slots, fontSize8) built for a different job, tags and tool pickers, and is structurally too different to bend into this shape.

One thing to do: Hinted uses pro/bold + pro/subtle, not a warning token, because the system has no feedback/warning/bold+subtle family, only a lone text/warning scalar. That's a real gap worth filling rather than a mistake to fix here. Revealed is my own addition, inferred from the brief's hint-ladder mechanic (reveal after 2 hints), not copied from a live screen, so confirm it's the right 4th status. Also worth noting: design-system.md's assumed vocabulary was "unaided," but the real screen copy says "Recalled," so that doc should be corrected. (Corrected here: this file's Section 1 chips entry above still says "unaided, hinted, revealed, skipped" describing chips' original intended use; that line describes a past assumption about chips, not tag, and is left as-is since chips itself is untouched. tag uses the real copy: Recalled, Hinted, Revealed, Skipped.)
