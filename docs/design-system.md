...
**sessionStats (Summary's XP/Score/time row)** — same hand-built, non-componentized territory as badge above. Built into code 2026-09-16, then removed the same day after Mia's review ("doesn't look right") — back to flagged-not-built status. What specifically looked wrong isn't captured here; see SPEC.md's Open section before attempting it again.
