...
**Almost nothing here is a bound token.** \`get_variable_defs\` returns exactly two entries for the whole component — \`interactive/primary\` and \`background/input\` — confirmed, not assumed. Everything else (the 40px pill radius, 16px/4px padding, the 24×24 icon box, the pause bars' exact size/position, every waveform bar's width/gap/corner-radius, and all 37 individual bar heights + opacities) is an unbound literal, reproduced as Figma's own static illustration rather than silently bound to a same-numbered token or invented as live audio data — this sprint has no real audio to visualize.
